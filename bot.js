...
this.bot.on('message', async (msg) => {
  ...
  // === ДОБАВЬТЕ ЭТОТ БЛОК СНАЧАЛА ===
  // Проверяем, находится ли пользователь в процессе редактирования поля
  if (userState && userState.action === 'editing_field' && userState.step === 'enter_new_value') {
    ...
    if (text === '❌ Отменить редактирование') {
      ...
      // Возвращаемся к меню редактирования места
...
async processFieldEdit(chatId, text, state) {
  try {
    ...
    if (editingField === 'latitude' || editingField === 'longitude') {
    ...
    if (editingField === 'average_price') {
      const numValue = parseInt(newValue);
      if (isNaN(numValue) || numValue < 0) {
        await this.bot.sendMessage(chatId, '❌ Пожалуйста, введите корректную сумму (например: 1000).');
        return;
      }
      newValue = numValue;
    }
    ...
    if (result.success) {
      ...
      await this.bot.sendMessage(chatId, `✅ Поле "${editingField}" успешно обновлено!`, {
        parse_mode: 'Markdown',
        reply_markup: { remove_keyboard: true }
      });
